    ...
    <main class="au-profile">
        ...
        <div class="au-profile__content  page-container">
            <div class="au-profile__loyalty-box">
                ...
                {/if}
                {'!getLoyaltyInfoFromMaxma' | snippet}
                <div class="au-profile__loyalty  au-profile__loyalty_bonuses {if $_modx->user.join_loyalty}active{/if}">
                    <h2 class="au-h2  au-profile__loyalty-title">
                        ...
                        <span class="au-bonuses__count">
                            <span>{'!msMultiCurrencyPriceFloor' | snippet : ['price' => $_modx->getPlaceholder('loyalty.bonus')]}</span>
                            {$_modx->getPlaceholder('loyalty.bonus') | declension : ('stik_declension_bonuses' | lexicon)}
                        </span>
                    </h2>
                    <div class="au-profile__loyalty-row">
                        ...
                        <b class="au-profile__loyalty-value" id="current_level">{('stik_loyalty_level_name_' ~ $_modx->getPlaceholder('loyalty.level')) | lexicon}</b>
                    </div>
                    <div class="au-profile__loyalty-range">
                        <div class="au-profile__loyalty-slider" style="width: {$_modx->getPlaceholder('loyalty.progressPercent') ?: 100}%;"></div>
                    </div>
                    <div class="au-profile__loyalty-row">
                        <span class="au-profile__loyalty-col">{'stik_profile_loyalty_purchase_amount' | lexicon}</span>
                        <b class="au-profile__loyalty-value"><span id="amount">{'!msMultiCurrencyPriceFloor' | snippet : ['price' => $_modx->getPlaceholder('loyalty.levelSpent')]}</span> {$_modx->getPlaceholder('msmc.symbol_right')}</b>
                    </div>
                    <div class="au-profile__loyalty-row  au-profile__amount_level">
                        ...
                        <b class="au-profile__loyalty-value"><span id="amount_level">{if $_modx->getPlaceholder('loyalty.remaining')}{'!msMultiCurrencyPriceFloor' | snippet : ['price' => $_modx->getPlaceholder('loyalty.remaining')]}</span> {$_modx->getPlaceholder('msmc.symbol_right')}{else}-{/if}</b>
                    </div>
