...
class stikAmoCRM
{
  ...
  public function createFormLead(array $data, array $amoFields)
  {
    ...
    $params = [
      [
        ...
        'pipeline_id' => (int)$this->config['orderPipelineId'],
        'created_at' => (int)($order ? strtotime($order['createdon']) : time()),
  ...
  public function createLeadWithContact(array $data, array $amoFields)
  {
    $customFields = [];
    foreach ($amoFields as $k => $v) {
      if (!isset($data[$k])) continue;
    ...
    $params = [
      [
        'name' => isset($data['form_name']) ? $data['form_name'] : $this->modx->lexicon('stikamocrm_form_name_new', ['date' => date('d.m.Y H:i:s')]),
    ...
    if ($contact = $this->searchContact($data['phone'])) {
      $params[0]['_embedded'] = [
        'contacts' => [
          [
            'id' => $contact,
    ...
    if (isset($result['errorCode'])) {
      $this->modx->log(MODX_LOG_LEVEL_ERROR, 'stikAmoCRM createLeadWithContact error: ' . print_r($result, 1));
    } else {
      // return $result;
      // return $result['_embedded']['leads'][0]['id'];
      return $this->createTaskToLead($result[0]['id'], $data['order']?:'');
    }
    return false;
  }

  public function createTaskToLead(int $entity_id, string $text)
  {
    $params = [
      [
        'text' => $text,
    ...
    $response = $this->modRest->post('api/v4/tasks', $params, $this->header_token);
    $result = $response->process();
    if (isset($result['errorCode']) || !isset($result['_embedded']['tasks'][0]['id'])) {
      $this->modx->log(MODX_LOG_LEVEL_ERROR, 'stikAmoCRM createTaskToLead error: ' . print_r($result, 1));
    } else {
      return $result['_embedded']['tasks'][0]['id'];
    }
    return false;
  }
